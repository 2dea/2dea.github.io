...
(function(factory){
	...
	var datepickerCalendar3 = [
		...
		'</div>',
		'<hr class="datepicker-calendar-masking" />'
	];
	...
	Datepicker.prototype.populateDaysCalendar = function() {
		...
		var curDay = 1;
		var rowCount = 1;
		this.$monthObj.html('<span class="datepicker-month-year">' + this.year + '<\/span>' + ' ' + '<span class="datepicker-month-month">' + this.locales.month_names[this.month] + '<\/span>');
		...
		for ( ; numEmpties > 0; numEmpties--) {
			gridCells += '\t\t<td class="empty"><span>' + (numPrevDays - numEmpties + 1) + '</span></td>\n';
		}
		...
		for (curDay = 1; curDay <= numDays; curDay++) {
			...
			gridCells += ' headers="day' + weekday + '-header-' + this.id + '" role="gridcell" tabindex="-1" aria-selected="false"><span>' + curDay;
			gridCells +=  '</span></td>';
		...
		while (weekday != lastDayOfWeek) {
			gridCells += '\t\t<td class="empty"><span>' + (++numEmpties) + '</span></td>\n';
			weekday = (weekday + 1) % 7;
		}
		gridCells += '\t</tr>';
	...
	Datepicker.prototype.populateMonthsCalendar = function() {
		...
		for (curMonth = 0; curMonth < 12; curMonth++) {
			...
			gridCells += ' role="gridcell" tabindex="-1" aria-selected="false"><span>' + this.locales.month_names_abbreviated[curMonth];
			gridCells +=  '</span></td>';
	...
	Datepicker.prototype.populateYearsCalendar = function() {
		...
		for (var curYear = startYear; curYear <= endYear; curYear++) {
			...
			gridCells += ' role="gridcell" tabindex="-1" aria-selected="false"><span>' + curYear;
			gridCells +=  '</span></td>';
