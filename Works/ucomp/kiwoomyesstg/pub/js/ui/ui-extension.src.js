...
(function($, window, document, location, undefined) {
    ...
    onExt.pre = {
        ...
        "scrollable": ['', 'html', '.popup-layer ' + onExt.dom._.divbody, '.dialog-layer ' + onExt.dom._.divbody, '.ground-layer ' + onExt.dom._.divbody, '.dom-nav .nav-total >* >.label +*'],
...
//            }
    ...
    onExt.fn.$_forcedFocusArea = {
        callback: function() {
            ...
            /* $(document)
                ...
                .on('keydown.focusables.forcedFocusArea', '[aria-modal="true"], [aria-modal="true"] :focus, .FN_forcedFocusArea, .FN_forcedFocusArea :focus', function(evt) {
            ...
            /* var _sel = [
                ...
                onExt.get.pushArraySel(onExt.pre.focusable, '.FN_forcedFocusArea ')
            ];

            console.log( _sel.join() ); */

            $(document)
                .on('keydown.focusables.forcedFocusArea', '[aria-modal="true"], .FN_forcedFocusArea', function(evt) {
    ...
    onExt.ui.$_scrollableArea = {
        ...
        refresh: function() {
            ...
            if( $popbody.length ) {
                var $popup = $popbody.closest('.popup-layer, .dialog-layer, .ground-layer');
    ...
    onExt.fn.$_listbox = {
        ...
        callback: function() {
            ...
            $(document)
                .on('click.checks.' + tag, '.ground-layer.type-select .m-listbox input', function(evt) {
//                .on('mouseup.checks.' + tag, '.ground-layer.type-select .m-listbox input', function(evt) {
                    ...
                    var $popup = $check.closest('.ground-layer'), id = $popup[0].id;
            ...
            /* $('.js_' + tag + '-wrap')
                .each(function(idx) {
                    ...
                    if( $wrap.children().hasClass('js_' + tag + '-wrap') ) {
                        ...
                        $wrap
                            .children(':has([data-js="FN_' + tag + '-item"])')
                                .addClass('FN_' + tag)
                        ;
                    }
                })
        ...
        callback: function() {
            ...
            $(document)
                ...
                .on('click.rows.' + tag, '.FN_' + tag, function(evt) {
                    var _item = '[data-js~="FN_' + tag + '-item"]';
        ...
        "fireReset": function(_target) {
            ...
            $fires
                .each(function(idx) {
                    ...
                    // if( onExt.fn.hasAttr(node, 'data-js', 'FN_bindsRow-item') ) {
                    if( $fire.is('[data-js~="FN_bindsRow-item"]') ) {
...
//                            ;
    ...
    onExt.fn.$_popup = {
        ...
        "hide": function(_target) {
            ...
            if( !$popup.length ) {
                ...
                if( !($popup.hasClass('ground-layer type-select') && $popup.find('input:checked').length) ) {
                    $popup
                ...
                setTimeout(function() {
                    $popup
                        //.removeAttr('data-fn')
                        .removeClass('js_hiding js_show FN_forcedFocusArea js_expand')
                        .find('.layer-body')
                            .addClass(function() {
                        ...
                        .closest(_root)
                            .removeClass(modal)
                            .addClass(function() {
                                var $dims = $('[data-js-layer-index]').not(_target).find('>.c-masking');
        ...
        "show": function(_target, _fire) {
            ...
            if( !$popup.length ) {
                ...
                $popup
                    ...
                    .addClass('js_show js_showing FN_forcedFocusArea')
                    .find('.js_scrollable:not(.js_scrollready)')
                        // .on('scroll', onExt.fn.$_detectScroll.top)
                        .on('scroll', onExt.fx.reqAnimate( onExt.get.$.Document.scroll ))
                        .addClass('js_scrollready')
                        .end()
                    .closest(_root)
                        .addClass(function() {
                            var $dim = $popup.find('>.c-masking');
        ...
        callback: function() {
            $(document)
                .on('click.popups.close', '.layer-tail .close', function(evt) {
                    ...
                    var $target = $(this).closest('.popup-layer, .dialog-layer, .ground-layer');

                    onExt.fn.$_popup.hide( '#' + $target[0].id );
                })
                .on('keydown.popups.close', function(evt) {
                    if( /\b27\b/.test( onExt.dom.onKey(evt) ) ) {
                        var $target = onExt.pre.$.popbody.closest('.popup-layer, .dialog-layer, .ground-layer');
                ...
                .on('mouseup.modals.close', '.c-masking', function(evt) {
                    var bind = this, $dim = $(bind);

                    if( $dim.parent().is('.popup-layer, .ground-layer') ) {
            ...
            try {
                if( "ontouchstart" in window ) {
                    ...
                    $('.ground-layer .layer-tail .close, .ground-layer .c-masking')
                        .Touchmove({
                            swipe: function(evt, dir) {
                                var bind = this, $bind = $(bind);
                                var $layer = ( $bind.is('.c-masking') ) ? $bind.parent() : $bind.closest('.layer-wrap').parent();
    ...
    onExt.fn.$_load = {
        ...
        "start": function(msg) {
            ...
            var _load_ = '<div role="alert" aria-busy="true" class="dom-loading type-univ ANI_ellipsis"><div class="node-wrap"><div class="div-loading"><i aria-hidden="true"><span></span><span></span><span></span><span></span></i><p class="messages">Loading...</p></div></div><hr class="c-masking" /></div>';
            var $load = $(_load_);
        ...
        "refStart": function(msg) {
            var _load_ = '<div role="alert" aria-busy="true" class="dom-loading type-ref"><div class="node-wrap"><div class="div-loading"><i aria-hidden="true"></i><p class="messages">Loading...</p></div></div><hr class="c-masking" /></div>';
            var $load = $(_load_);
    ...
    onExt.fn.$_stepsMove = {
        ...
        "push": function(num) {
            ...
            var $popup = onExt.pre.$.popbody.closest('.popup-layer, .dialog-layer, .ground-layer');
    ...
    onExt.fn.$_navCtrl = {
        ...
        callback: function() {
            $(document)
                ...
                .on('pointerup.overlay.navCtrl', '.dom-nav >.c-masking', function(evt) {
                    evt.stopPropagation();
    ...
    onExt.ui.$_carousel = {
                ...
                .each(function(idx) {
                    ...
                    $swiper
                        .find( onExt.get.pushArraySel('focusable', '.swiper-slide-duplicate ') )
                            ...
                            .end()
                        // .find('.swiper-button-play', '.swiper-controls')
                        .find('.swiper-controls .swiper-button-play')
                            .attr({
        ...
        callback: function() {
            $(document)
                ...
                .on('mouseup', '.UI_carousel.swiper-container-initialized .swiper-button-play', function(evt) {
                    evt.preventDefault();
                    ...
                    $wrap
                        .find(_panel)
                            .each(function(idx) {
                                ...
                                var $label = ( $fire.is('[data-js~="FN_bindsRow-item"]') ) ? $fire.closest('.FN_bindsRow') : $fire;
    ...
    onExt.fn.$_detectResolution = {
        ...
        create: function() {
            ...
            var style = 'font-size:9pt; position:fixed; left:-1in; top:-3ex; width:1in; height:100%; height:100vh; overflow-y:scroll; visibility:hidden;';
...
//        },
    ...
    onExt.fn.$_detectScroll = {
        ...
        "top": function(obj) {
            ...
            var cnUp = 'js_scrollup';
            var cnDown = 'js_scrolldown';
            ...
            if( !isWin ) {
                ...
                _topDatums_ = ( /\b(fixed|sticky|-webkit-sticky)\b/i.test( $header.css('position') ) ) ? $header.innerHeight() / 2 : 0;
...
//                ( isWin ) && $root.removeClass(cnScroll); //###
...
//                }
            ...
            /* if( scrollY > 0 ) {
                $root.addClass(cnScroll);

            } else {
                $root.removeClass(cnScroll);
            } */

//            ( isWin ) && $root.addClass(cnScroll); //###
        }
    };
...
jQuery(window).on('load', function(evt) {
    ...
    $(this)
        .on({
            ...
            scroll: req( win.scroll ),

            orientationchange: win.orientationchange,

            resize: req( win.resize )
        })
        // .trigger('hashchange')
