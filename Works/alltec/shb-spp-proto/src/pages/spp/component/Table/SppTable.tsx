...
const SppTable = forwardRef(<T extends object = any>({ heightSectionTest, heightSectionFixed=true, heightSectionBasis, ...props }: SppTableProps<T>, ref: any) => {
  ...
  const renderServerPagination = () => {
    ...
    const pageChange = (page: number, nextPageSize: number) => {
      ...
      if (props?.onChange) props?.onChange(p, {}, {}, { action: 'paginate', currentDataSource: [] });
    };
  ...
  const setClassName = (record: any, index: number) => {
    return index === props.selectedRowIndex ? 'row-highlight' : '';
  };

  const setSelectedClassName = (record: any, index: number) => {
    return index === selectRowIndex ? 'row-highlight' : '';
  };
