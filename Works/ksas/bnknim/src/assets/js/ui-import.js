    ...
    if( cssPath || jsPath ) {
        ...
        if( !(window.CSS && CSS.supports('top', 'var(--var)') || window.cssVars) ) {
            ...
            if( !/^https?:$/i.test( location.protocol ) ) {
                ...
                window.addEventListener('load', function(evt) {
                    // attachResource('cssVars({ onComplete: function(cssText, styleElms, cssVariables, benchmark) { document.body.className += \' js_cssVars-complete\', window.jQuery && window.onExt && onExt.page && onExt.dom.resize() || window.dispatchEvent( new Event('resize') ); } });', { "defer": "defer" });
                    attachResource('cssVars({ onComplete: function(cssText, styleElms, cssVariables, benchmark) { document.body.className += \' js_cssVars-complete\', window.jQuery && window.onExt && onExt.page && onExt.dom.resize(); } });', { "defer": "defer" });
                }, false);
            }
        }
