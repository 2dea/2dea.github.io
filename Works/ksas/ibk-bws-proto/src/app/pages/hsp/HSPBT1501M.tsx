/**
 * @description 중요용지·용도품(본부) > 영업점배송관리 > 영업점미인수내역조회
 */
...
function HSPBT1501M() {
  ...
  // BreadCrumb
  const paths: MenuItem[] = [{ label: '홈' }, { label: '중요용지·용도품(본부)' }, { label: '영업점배송관리' }, { label: '영업점미인수내역조회' }];
                      ...
                      <table className="p-datatable-table p-datatable-scrollable-table">
                        ...
                        {[...Array(24)].map((e, idx) => (
                          <tr key={idx} className={`${idx === 0 ? 'p-highlight' : ''}`}>{/* 그리드 최초 진입시 첫번째 행 tr.p-highlight <$tr.trigger('click')> { @DEV } */}
                            ...
                            <td>{idx + 1}</td>
                            <td>0001</td>
                            <td className="g-start">송파카드팀</td>
                            <td><span className="o-digit type-date">2025-12-25</span></td>
                            <td>2237</td>
                            <td className="g-start">일상의 기쁨 체크(후불교통_MC_신디자인)</td>
                            <td className="g-end">500</td>
                            <td className="g-end">79,500</td>
                            <td>0810</td>
                          </tr>
                        ))}
                        </tbody>
